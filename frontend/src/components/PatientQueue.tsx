...
export function PatientQueue({ onNavigate }: PatientQueueProps) {
  ...
  const highPriority = queue.filter((p) => p.risk_level === 'critical' || p.risk_level === 'high').length
          ...
          <div className="card">
            ...
            <div className="card-body" style={{ padding: 0, overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', minWidth: 800 }}>
                <thead>
                  <tr style={{ borderBottom: '1px solid var(--border-light)', color: 'var(--text-muted)', fontSize: 13, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                    ...
                    >
                      <td style={{ padding: '16px 24px', fontWeight: 500, color: 'var(--text-primary)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                          <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'var(--blue-50)', color: 'var(--blue-600)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 600 }}>
                      ...
                      <td style={{ padding: '16px 24px' }}>
                        <span className={`badge badge-${patient.risk_level === 'critical' ? 'red' : patient.risk_level === 'high' ? 'amber' : patient.risk_level === 'moderate' ? 'blue' : 'green'}`}>
                      ...
                      <td style={{ padding: '16px 24px', textAlign: 'right' }}>
                        <button 
                          type="button" 
                          onClick={(e) => { e.stopPropagation(); onNavigate('patient-profile', patient.id); }}
                          style={{ 
                            padding: '6px 12px', 
