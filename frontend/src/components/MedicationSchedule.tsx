...
export function MedicationSchedule({ medications }: MedicationScheduleProps) {
  ...
  return (
    <div>
      {/* Summary bar */}
      <div className="flex items-center justify-between mb-4" style={{ padding: '10px 14px', background: 'var(--slate-50)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)' }}>
        <span className="text-sm font-semibold" style={{ color: 'var(--text-secondary)' }}>
          Today's Progress
        </span>
        <span className="text-sm font-semibold" style={{ color: taken.length === medications.length ? 'var(--green-500)' : 'var(--amber-500)' }}>
      ...
      {pending.length > 0 && (
        <div style={{ marginBottom: 12 }}>
          <div style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--amber-500)', marginBottom: 8 }}>
            ⏰ Due Today
          </div>
      ...
      {taken.length > 0 && (
        <div>
          <div style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--green-500)', marginBottom: 8 }}>
            ✓ Taken Today
          </div>
          <div className="medication-list">
            {taken.map(med => (
  ...
  return (
    <div className={`medication-item ${status}`}>
      ...
      <div className="med-info">
        ...
        {med.purpose && (
          <span className="med-purpose-tag">{med.purpose}</span>
        )}
        {med.instructions && (
