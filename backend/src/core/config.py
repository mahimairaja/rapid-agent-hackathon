...
class Config(BaseSettings):
    ...
    # Voice (F6). GEMINI_LIVE_MODEL must be a Gemini Live native-audio model that
    # advertises bidiGenerateContent; the voice agent reuses the same
    # tools/prompts as the text agent but with this model. The earlier
    # gemini-2.0-flash-live-001 default is not available on this API key (the
    # live socket closes with APIError 1008), so the native-audio model is the
    # working default. Confirm the exact id against availability at deploy time.
